...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['confirm_order'])) {
    ...
    try {
        ...
        if (mysqli_query($conn, $sql_order)) {
            ...
            // B. Lưu chi tiết sản phẩm (LẤY GIÁ BÁN TỪ BẢNG GIACA)
            foreach ($_SESSION['cart'] as $product_id => $quantity) {
                // CHỈNH SỬA TẠI ĐÂY: Truy vấn giaca để tính giá bán
                $sql_p = "SELECT cost_price, profit_margin FROM giaca WHERE product_id = $product_id";
                $res_p = mysqli_query($conn, $sql_p);
                $p_data = mysqli_fetch_assoc($res_p);
                
                // Công thức tính giá bán thực tế
...
<body>
    ...
    <div class="cot_trai" style="<?php echo $success ? 'width: 100%; max-width: 600px; margin: 0 auto; float: none;' : ''; ?>">
      ...
      <form action="thanhtoan.php" method="POST">
          ...
          <div class="dia_chi_thanh_toan">
            ...
            <button type="submit" name="confirm_order" class="nut_thanh_toan" style="background:#333; color:#fff; border-radius:5px; margin-top:20px;">Thanh toán ngay</button>
          </div>
      </form>
      <?php endif; ?>
    </div>
      ...
      <div class="don_hang">
        ...
        if(isset($_SESSION['cart']) && is_array($_SESSION['cart'])) {
            foreach ($_SESSION['cart'] as $id => $qty): 
                // CHỈ CHỈNH SỬA TẠI ĐÂY: JOIN lấy giá thực tế từ giaca
        ...
        <div class="hang_san_pham">
          <img src="image/<?php echo $p['main_image']; ?>" alt="Sản phẩm" style="width:60px; height:60px; object-fit:cover;">
          <div style="flex:1; margin-left:15px;">
            <p style="margin:0; font-weight:bold;"><?php echo $p['name']; ?></p>
            <p class="mau" style="margin:0; font-size:13px; color:#666;">Số lượng: <?php echo $qty; ?></p>
          </div>
      ...
      document.addEventListener('DOMContentLoaded', function() {
          ...
          function updateFields() {
              ...
              if(selectedOpt.value === 'new') {
                  inHo.value = ''; inTen.value = ''; inAddress.value = ''; inWard.value = ''; inPhone.value = '';
                  inHo.focus();
              } else {
  ...
  <footer class="c" style="text-align:center; padding:30px 0;">
    <div class="f">
      <a style="font-size:12px; color:#666; margin:0 10px;">Chính sách hoàn tiền</a>
      <a style="font-size:12px; color:#666; margin:0 10px;">Vận chuyển</a>
      <a style="font-size:12px; color:#666; margin:0 10px;">Quyền riêng tư</a>
    </div>
  </footer>
</body>
</html>