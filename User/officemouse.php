...
// 1. TRUY VẤN CƠ BẢN (Category 3: Chuột văn phòng)
$sql = "SELECT * FROM san_pham WHERE category_id = 3";
...
// 3. LỌC THÔNG SỐ ĐỘNG (Tự động cập nhật Polling rate, Switch... từ Admin)
if (isset($_GET['specs']) && is_array($_GET['specs'])) {
    foreach ($_GET['specs'] as $key => $values) {
        if (!empty($values)) {
            ...
            // Tìm sản phẩm có thuộc tính $key mang giá trị nằm trong danh sách chọn
            $sql .= " AND id IN (SELECT product_id FROM thong_so WHERE spec_key = '$safe_key' AND spec_value IN ($val_list))";
        }
    }
}

// 4. LỌC GIÁ (Quy đổi triệu đồng)
...
<head>
  ...
  <link rel="stylesheet" href="css/officemouse.css"> 
  <link rel="stylesheet" href="css/bootstrap-icons.css">
  <title>Chuột Văn Phòng - Phong Cách Xanh</title>
</head>
<body>
  <header>
    <nav class="menu">
  ...
  <section class="mota">
    <img src="image/officemousebanne.webp" alt="" class="mota_img">
    <div class="mota-box">
      <p class="de">Chuột Văn Phòng Cao Cấp – Click Êm, Form Công Thái Học, Làm Việc Mượt</p>
      <p>Chuột văn phòng cao cấp từ Logitech & Razer – tối ưu cho môi trường chuyên nghiệp: thao tác êm, form công thái học, cuộn siêu mượt, pin lâu. Đảm bảo hiệu suất và cảm giác sử dụng khác biệt so với chuột phổ thông.</p>
      <p><strong>Click êm – không gây tiếng:</strong> Lý tưởng cho văn phòng mở, họp call, không phiền đồng nghiệp.</p>
      <p><strong>Cuộn tài liệu siêu mượt:</strong> MagSpeed (Logitech) & SmartScroll (Razer) cho bảng tính, code dài.</p>
      <p><strong>Form công thái học:</strong> Giữ đúng tư thế cổ tay, giảm đau mỏi khi làm việc liên tục.</p>
    </div>
  </section>
    ...
    <div class="main">
      <div class="khung_thanh_loc">
        <form action="" method="GET" id="filter">
          ...
          <div class="filter-box">
            ...
            <div class="filter-content">
              ...
              $brand_query = "SELECT DISTINCT th.id, th.name FROM thuong_hieu th 
                              JOIN san_pham sp ON th.id = sp.brand_id WHERE sp.category_id = 3";
          ...
          <?php
          // Lấy danh sách các Spec Keys của Chuột văn phòng (ID 3)
          $keys_query = "SELECT DISTINCT spec_key FROM danhmuc_spec_keys WHERE category_id = 3"; 
          ...
          ?>
              <div class="filter-box">
                ...
                <div class="filter-content">
                  <?php
                  // Quét Database lấy các giá trị thực tế đang có ngoài kho
                  $val_query = "SELECT DISTINCT spec_value FROM thong_so 
                                WHERE spec_key = '$current_key' 
                                AND product_id IN (SELECT id FROM san_pham WHERE category_id = 3)";
          ...
          <div class="filter-box">
            ...
            <div class="filter-content">
              ...
              <label><input type="checkbox" name="price[]" value="2-3" onchange="this.form.submit()" <?= (isset($_GET['price']) && in_array('2-3', $_GET['price'])) ? 'checked' : '' ?>> Từ 2 đến 3 triệu</label><br>
              <label><input type="checkbox" name="price[]" value="3-5" onchange="this.form.submit()" <?= (isset($_GET['price']) && in_array('3-5', $_GET['price'])) ? 'checked' : '' ?>> Từ 3 đến 5 triệu</label><br>
            </div>
          </div>
        </form>
      </div>
