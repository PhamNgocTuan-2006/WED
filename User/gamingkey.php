...
// 1. CÂU LỆNH TRUY VẤN CƠ BẢN (Category 2: Bàn phím Gaming)
$sql = "SELECT * FROM san_pham WHERE category_id = 2";
...
// 3. LỌC THÔNG SỐ ĐỘNG (Tự động cập nhật Loại sản phẩm, Layout, v.v...)
...
<head>
    ...
    <link rel="stylesheet" href="css/keygaming.css">
    <link rel="stylesheet" href="css/bootstrap-icons.css">
    <title>Bàn Phím Gaming - Phong Cách Xanh</title>
</head>
...
<body>
    ...
    <section class="mota">
        <img src="image/gamingkeybanner.webp" alt="" class="mota_img">
        <div class="mota-box">
            <p class="de">Bàn phím cơ & HE – Gõ đã tay, phản hồi chuẩn</p>
            <p><strong>Bàn phím cơ và bàn phím Hall Effect</strong> đều là lựa chọn đỉnh cho game thủ lẫn dân văn phòng kỹ tính. Ở đây, <strong>nhà Xanh</strong> chỉ tuyển những chiếc phím gõ sướng – build chắc – phản hồi tốt, bất kể bạn cần combo nhanh hay code 8 tiếng liền.</p>
            <p><strong>Switch cơ & Hall Effect:</strong> Đều cho cảm giác nhấn rõ, bấm chính xác</p>
            <p><strong>Layout đa dạng:</strong> Từ 60%, TKL đến fullsize</p>
            <p><strong>Tùy chọn:</strong> Hot-swap, không dây, keycap đẹp – có đủ</p>
            <p>Dù bạn chơi game, viết code hay đơn giản là mê tiếng “click” đã tai – ở đây luôn có chiếc phím đúng gu bạn cần.</p>
        </div>
    </section>
                ...
                <form action="" method="GET" id="filter">
                    <div class="filter-box">
                        ...
                        <div class="filter-content">
                            <?php
                            $brand_query = "SELECT DISTINCT th.id, th.name FROM thuong_hieu th JOIN san_pham sp ON th.id = sp.brand_id WHERE sp.category_id = 2";
                    ...
                    <?php
                    // Lấy các Spec Key dành riêng cho Bàn phím Gaming (ID 2)
                    $keys_query = "SELECT DISTINCT spec_key FROM danhmuc_spec_keys WHERE category_id = 2"; 
                    ...
                    ?>
                        <div class="filter-box">
                            ...
                            <div class="filter-content">
                                <?php
                                // Quét bảng thong_so để lấy giá trị thực tế đang có
                                $val_query = "SELECT DISTINCT spec_value FROM thong_so 
                                              WHERE spec_key = '$current_key' 
                                              AND product_id IN (SELECT id FROM san_pham WHERE category_id = 2)";
                    ...
                    <div class="filter-box">
                        ...
                        <div class="filter-content">
                            ...
                            <label><input type="checkbox" name="price[]" value="5000000-10000000" onchange="this.form.submit()" <?= (isset($_GET['price']) && in_array("5000000-10000000", $_GET['price'])) ? 'checked' : '' ?>> Trên 5 triệu</label><br>
                        </div>
                    </div>
                </form>
            </div>
